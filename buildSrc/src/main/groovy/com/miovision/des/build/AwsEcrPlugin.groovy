package com.miovision.des.build
...
class AwsEcrPlugin implements Plugin<Project> {
    private static final Pattern AWS_ECR_URL = ~/([0-9A-Za-z]+)\.dkr\.ecr\.[a-zA-Z0-9-]+\.amazonaws\.com/
    ...
    protected String findRepository(RegistryCredentialsAware registryCredentialsAware) {
        ...
        project.logger.warn("Skipping docker credentials for ${registryCredentialsAware} because we do not know how to find a repository for this type (${registryCredentialsAware.class}")
        null
    }
    ...
    protected void configureRegistryCredentials(RegistryCredentialsAware registryCredentialsAware) {
        ...
        if (!info) {
            project.logger.warn("Skipping docker credentials for ${registryCredentialsAware} because repository '${repository}' is not AWS ECR")
            return
        }
    ...
    protected DockerRegistryCredentials findEcrCredentials(String registryId, String registryUrl) {
        def credFile = project.rootProject.file(".gradle/ecr.properties")
        ...
        if (tokens.authorizationData) {
            ...
            credFile.withOutputStream { props.store(it, "ECR Credentials for ${registryUrl}") }
